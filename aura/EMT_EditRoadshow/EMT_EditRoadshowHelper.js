({
    ...
    fetchRoadshowData:function(component)
    {
        var action = component.get("c.getRoadshowDetails");
        action.setParams({'recordId':component.get("v.recordId")});
        action.setCallback(this,function(response){
            var state = response.getState();
            if(state == "SUCCESS")
            {
                component.set("v.eventObject",response.getReturnValue());
                 
                if(!$A.util.isEmpty(response.getReturnValue().Primary_Contact_Lookup__c)) {
                   ...
                   // component.set("v.eventObject.CT_Event_primary_contact__c",returnData.CT_Event_primary_contact__c);
                    component.set("v.eventObject.CT_Event_primary_contact_email__c",response.getReturnValue().CT_Event_primary_contact_email__c);
                    component.set("v.primaryContactText",response.getReturnValue().CT_Event_primary_contact__c);
                    
                }
                 if(response.getReturnValue().CT_Reaccuring_event__c==true)
                {
                ...
                if(response.getReturnValue().Capturing_leads_at_the_event__c) {
                ...
                component.set("v.eventObject.CT_NDA_Customer_meetings__c",response.getReturnValue().CT_NDA_Customer_meetings__c);
                
                if(response.getReturnValue().CT_Hands_on_tutorial__c == false)
                {
                ...
                if(response.getReturnValue().CT_Symposia__c==false)
                {
            ...
            {
                console.log("Callback Of getRoadshowDetails is failed");
            }
        });
        $A.enqueueAction(action);
    },
    ...
    checkFieldValidation:function(component,auraId) {
        ...
        if(auraId == 'basic_info_field3') {	
            if(component.get("v.data").length==0) {
                errorMessage = "Please select atleast one cluster in the \'Event Focus\' tab before saving the event" ;
        ...
        var isValid = fieldstoValidate.reduce(function(initialValidCheck,inputElement) {
            ...
            if(inputElement.get("v.label")=='Budget in local currency') {
                ...
                if(auraId=="basic_info_field1" && (component.get("v.eventObject.Event_type__c")=="" ||component.get("v.eventObject.Event_type__c")=="--- None ---") && i==0) {
                    errorMessage = "Please complete \'Event type\' in the \'Type/Size\' before saving the event";
                    var invalidFieldList =  component.get("v.invalidField");
                    invalidFieldList.push(errorMessage);
                    component.set("v.invalidField",invalidFieldList);
                    component.set("v.isValid",true);
                ...
                if(auraId=="basic_info_field1" && (component.get("v.eventObject.CT_External_event_subtype__c")=="" ||component.get("v.eventObject.CT_External_event_subtype__c")=="--- None ---") && j==0) {
                    errorMessage = "Please complete \'Event Sub type\' in the \'Type/Size\' before saving the event";
                    var invalidFieldList =  component.get("v.invalidField");
                    invalidFieldList.push(errorMessage);
                    component.set("v.invalidField",invalidFieldList);
                    component.set("v.isValid",true);
            ...
            if(auraId=="basic_info_field1" && (component.get("v.eventObject.external_event_subtypes__c")=="" ||component.get("v.eventObject.external_event_subtypes__c")=="--- None ---") && k==0) {
                    errorMessage = "Please complete \'External event subtype\' in the \'Type/Size\' before saving the event";
                    var invalidFieldList =  component.get("v.invalidField");
                    invalidFieldList.push(errorMessage);
                    component.set("v.invalidField",invalidFieldList);
                    component.set("v.isValid",true);
   ...
   /*removeBook: function (component, row) {
        ...
        console.log("rows.splice"+rows.splice(rowIndex, 1));
